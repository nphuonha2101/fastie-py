...
FastAPI documentation is available at `/docs`, `/redoc`, and `/openapi.json`.
The v1 router is mounted under `/api/v1`; additional routers such as v2 or v3
are created and mounted on demand, so features can migrate incrementally while
older clients keep using v1. Resources generated by `fastie make resource` are
registered in v1 by default.

For a ready-to-run local container stack with PostgreSQL, Redis, a non-root
application container, and a one-shot migration service, run Docker setup from
the local Fastie checkout. It auto-detects the checkout while the package is
unpublished:
...
For a private package registry, use `--fastie-package your-package==version`.

## Generate a resource

```bash
fastie make resource Product --fields "name:str,price:decimal,is_active:bool"
# Add another API contract while reusing the same model and migration history.
fastie make resource Product --version v3 --reuse-model --fields "name:str,price:decimal,is_active:bool"
# Use a separate ORM model/table when the persistence contract is different.
...
project. `--version vN` accepts any positive numeric version, creates the
version router if needed, and keeps version-specific schemas/routes separate.
Use `--reuse-model` when adding another API version for an existing resource.
Use `--model ProductV2` to explicitly generate a separate ORM model and table;
review and apply its migration separately.
Review generated code before applying the migration.
...
- `fastie setup docker`: Generate a Dockerfile, Compose stack, and Docker environment template. It detects a local checkout; use `--local-source <path>` or `--fastie-package <requirement>` explicitly when needed.
...
- `fastie make resource <Name> [--version vN] [--reuse-model] [--model ModelName]`: Generate a versioned CRUD path; v1 is the default.
