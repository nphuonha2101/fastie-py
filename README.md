...
Versioned routers are mounted independently under `/api/v1` and `/api/v2`, so
features can migrate incrementally while older clients keep using v1. Resources
generated by `fastie make resource` are registered in v1 by default; future
versions such as v3 are created automatically when requested.
...
- `fastie make resource <Name> [--version vN] [--reuse-model]`: Generate a CRUD path for a version; v1 is the default.
