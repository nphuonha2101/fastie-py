...
FastAPI tự sinh tài liệu tại `/docs`, `/redoc` và `/openapi.json`. Router v1 và
v2 được mount độc lập dưới `/api/v1` và `/api/v2`, nên có thể migrate từng
feature; resource tạo bằng `fastie make resource` mặc định vào v1. Các version
tiếp theo như v3 sẽ được tạo tự động khi cần.
...
review code và migration trước khi chạy production.
...
- `fastie make resource <Name> [--version vN] [--reuse-model]`: Tạo CRUD cho version chỉ định; mặc định là v1.
