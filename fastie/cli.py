...
def _validate_api_version(ctx, param, value):
    """Accept any positive numeric API version (v1, v2, v3, ...)."""
    normalized = value.strip().lower()
    if not re.fullmatch(r'v[1-9]\d*', normalized):
        raise click.BadParameter(
            'must match vN, for example v1, v2, or v10',
            ctx=ctx,
            param=param,
        )
    return normalized
...
@click.option(
    '--fastie-package',
    help='Package requirement to install when a local Fastie source is unavailable.',
)
@click.option('--force', is_flag=True, help='Overwrite existing Docker setup files.')
def setup_docker(database, workers, local_source, fastie_package, force):
    """Generate a production-oriented Docker Compose stack."""
    project_root = Path.cwd()
    database = database.lower()

    if local_source and fastie_package:
        raise click.UsageError('Use only one of --local-source or --fastie-package.')
    if local_source is None and fastie_package is None:
        local_source = _detect_local_fastie_source()
    if local_source is None and fastie_package is None:
        raise click.ClickException(
            "Fastie source was not detected. Pass --local-source /path/to/fastie "
            "or --fastie-package your-private-package==version."
        )

    files = _docker_setup_files(database, workers, fastie_package=fastie_package)
    ...
    if local_source:
        ...
        files = _docker_setup_files(
            database,
            workers,
            local_wheel_name,
            fastie_package=fastie_package,
        )
...
def new(project_name, database, path):
    ...
    try:
        ...
        shutil.copytree(stubs_path, project_path, ignore=shutil.ignore_patterns(
            ...
            'app/api', 'app/repositories', 'app/services'
        ))
...
@click.option('--auto', is_flag=True, help='Use Alembic autogenerate for the migration')
def make_migration(name, table, empty, auto):
    ...
    if auto:
        if name:
            ...
            fastie_console.step(f"Using migration name: {message}")
        elif empty:
...
@click.option(
    '--version',
    'api_version',
    callback=_validate_api_version,
    default='v1',
    show_default=True,
    help='API version that owns the generated route and schemas (v1, v2, v3, ...).',
)
@click.option(
    '--reuse-model',
    is_flag=True,
...
def make_resource(name, fields, migrate, api_version, reuse_model, model_name, force):
    ...
    try:
        ...
        files = {
            ...
            response_root / snake_name / f"{snake_name}_response_schema.py":
                _generate_resource_response_schema(name, field_specs),
            route_path: _generate_resource_router(
                name,
                field_specs,
        ...
        init_files.add(Path('app/routes/__init__.py'))
        ...
        if version_router_ready and _register_resource_route(name, api_version):
            ...
            fastie_console.info(
                f"Add {class_name} router to app/routes/{api_version}.py if this project was created by an older Fastie version."
            )
...
def _detect_local_fastie_source():
    """Return the local Fastie checkout when the CLI runs from source."""
...
"""
        ...
        package_spec = fastie_package or f'fastie-py=={FASTIE_VERSION}'
        build_block = f"""    build:
      context: .
      args:
        FASTIE_PACKAGE: ${{FASTIE_PACKAGE:-{package_spec}}}
"""
        package_arg = f'ARG FASTIE_PACKAGE={package_spec}\n\n'
...
def _ensure_api_version_router(api_version):
    """Create and mount a version router when a project does not have it yet."""
    routes_dir = Path('app/routes')
    routes_file = routes_dir / f'{api_version}.py'
    api_file = routes_dir / 'api.py'
    if not routes_dir.is_dir() or not api_file.is_file():
        return False

    content = api_file.read_text(encoding='utf-8')
    import_marker = '# Fastie version routes - generated imports'
    include_marker = '# Fastie version routes - generated includes'
    if import_marker not in content or include_marker not in content:
        return False

    if not routes_file.exists():
        routes_file.write_text(
            f'''"""API router for version {api_version}."""
...
{api_version}_router = APIRouter(prefix="/{api_version}")
...
''',
    ...
    import_line = f'from app.routes.{api_version} import {api_version}_router'
    include_line = f'api_router.include_router({api_version}_router)'
...
def _register_resource_route(name, api_version='v1'):
    """Register a generated router in the version router marker section."""
    routes_file = Path(f'app/routes/{api_version}.py')
    if not routes_file.exists():
        return False
    ...
    import_line = (
        f'from app.routes.resources.{api_version}.{snake_name} '
        f'import router as {snake_name}_router'
    )
    include_line = (
        f'{api_version}_router.include_router({snake_name}_router, '
